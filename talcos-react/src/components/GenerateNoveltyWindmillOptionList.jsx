...
import Style from "./styles/generate-novelty-windmill-option-list.module.css";

function GenerateNoveltyWindmillOptionList() {
    ...
    const getData = async () => {
      try {
        ...
        const responseNews = await axios.get(
          `http://${localIP}:3000/novedades/listaapagadonovedad`,
          {
            params: {
              fecha: currentDate,
              turno,
  ...
  return (
    <>
      {loadingAlternative ? (
        <motion.div
          className={Style.generateNoveltyWindmillOptionListAlternative}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <motion.table
          className={Style.generateNoveltyWindmillOptionListMainTable}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5 }}
        >
          <thead
            className={Style.generateNoveltyWindmillOptionListMainTableHead}
          >
            <tr>
              <th>Molino</th>
              ...
              <th>Horómetro de apagado</th>
              <th>Horómetro de encendido</th>
            </tr>
          </thead>
          <tbody
            className={Style.generateNoveltyWindmillOptionListMainTableBody}
          >
            {novelty.map((novedad) => (
        ...
        <motion.div
          className={Style.generateNoveltyWindmillOptionListMainAlternative}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5 }}
        >
...
export default GenerateNoveltyWindmillOptionList;
