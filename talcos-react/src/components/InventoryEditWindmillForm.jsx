...
function InventoryEditWindmillForm() {
    ...
    const sendEditWindmill = async (e) => {
        ...
        try {
            await axios.put(`http://${localIP}:3000/Molinos`, {
                id_molino: idMolino,
                nombre_molino: nombreMolino === '' ? windmill.nombre_molino : nombreMolino,
                horometro_molino: horometroMolino === '' ? windmill.horometro_molino : horometroMolino,
            });
            ...
            if (error.response && error.response.data && error.response.data.error) {
                ...
                setServerError('Error al editar el molino. Por favor, inténtelo de nuevo.');
                setLoading(false);
            }
        }
    ...
    return (
        <>
            {SendStatus === true ? (
                <motion.div className={Style.inventoryEditWindmillFormAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    <h1>Molino editado con éxito</h1>
                </motion.div>
            ) : (
                <motion.form className={Style.inventoryEditWindmillForm} onSubmit={sendEditWindmill} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
