...
function InventoryDeleteShiftConfirmation() {
    ...
    const sendDeleteShift = async () => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.error) {
                ...
                setServerError('Error al eliminar el turno. Por favor, inténtelo de nuevo.');
                setLoading(false);
            }
        }
    ...
    return (
        <>
            {SendStatus === true ? (
                <motion.div className={Style.staffDeleteUserConfirmationAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    <h1>Turno eliminado con éxito</h1>
                </motion.div>
            ) : (
                <motion.div className={Style.staffDeleteUserConfirmation} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    ...
                    <footer className={Style.staffDeleteUserConfirmationFooter}>
                        ...
                        <button type='submit' onClick={() => sendDeleteShift()}>{loading ? (
