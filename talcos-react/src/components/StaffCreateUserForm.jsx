...
import Style from './styles/staff-create-user-form.module.css';

function StaffCreateUserForm() {
    ...
    return (
        <>
            {SendStatus === true ? (
                <motion.div className={Style.staffCreateUserFormAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    <h1>{perfil.nombre_perfil ? singularize(perfil.nombre_perfil.toLowerCase()) : 'Usuario'} creado con éxtio</h1>
                </motion.div>
            ) : (
                <motion.form className={Style.staffCreateUserForm} onSubmit={sendCreateUser} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
...
export default StaffCreateUserForm;