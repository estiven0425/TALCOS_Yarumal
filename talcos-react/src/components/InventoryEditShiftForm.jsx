...
function InventoryEditShiftForm() {
    ...
    const sendEditShift = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.error) {
                ...
                setServerError('Error al editar el turno. Por favor, inténtelo de nuevo.');
                setLoading(false);
            }
        }
    ...
    return (
        <>
            {SendStatus === true ? (
                <motion.div className={Style.inventoryEditShiftFormAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    <h1>Turno editado con éxito</h1>
                </motion.div>
            ) : (
                <motion.form className={Style.inventoryEditShiftForm} onSubmit={sendEditShift} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
