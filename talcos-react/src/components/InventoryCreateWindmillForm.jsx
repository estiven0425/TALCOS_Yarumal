...
function InventoryCreateWindmillForm() {
    ...
    const sendCreateWindmill = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.error) {
                ...
                setServerError('Error al crear el molino. Por favor, inténtelo de nuevo.');
                setLoading(false);
            }
        }
    };
    ...
    return (
        <>
            {SendStatus === true ? (
                <motion.div className={Style.inventoryCreateWindmillFormAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    <h1>Molino creado con éxito</h1>
                </motion.div>
            ) : (
                <motion.form className={Style.inventoryCreateWindmillForm} onSubmit={sendCreateWindmill} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
