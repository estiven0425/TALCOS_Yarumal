...
function InventoryCreateShiftForm() {
    ...
    const sendCreateShift = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.error) {
                ...
                setServerError('Error al crear el turno. Por favor, inténtelo de nuevo.');
                setLoading(false);
            }
        }
    };
    ...
    return (
        <>
            {SendStatus === true ? (
                <motion.div className={Style.inventoryCreateShiftFormAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                    <h1>Turno creado con éxito</h1>
                </motion.div>
            ) : (
                <motion.form className={Style.inventoryCreateShiftForm} onSubmit={sendCreateShift} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
