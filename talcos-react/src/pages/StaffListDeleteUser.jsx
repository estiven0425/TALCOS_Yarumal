...
function StaffListDeleteUser() {
    ...
    return (
        <ProtectedRoute>
            <motion.section className={Style.staffListDeleteUser} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                <header className={Style.staffListDeleteUserHeader}>
                    <h1>Seleccione un {perfil.nombre_perfil ? singularize(perfil.nombre_perfil.toLowerCase()) : 'Usuario'} para eliminar</h1>
                </header>
                <main className={Style.staffListDeleteUserMain}>
                    {usuario.length > 0 ? (
                        <motion.table className={Style.staffListDeleteUserMainTable} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                            <thead className={Style.staffListDeleteUserMainTableHead}>
                                <tr>
                                    <th>Nombre</th>
                            ...
                            <tbody className={Style.staffListDeleteUserMainTableBody}>
                                {usuario.map((usuario) => (
                                    <tr
                        ...
                        <motion.div className={Style.staffListDeleteUserMainAlternative} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                            <h2>No existen usuarios</h2>
                        </motion.div>
                    )}
                </main>
                <footer className={Style.staffListDeleteUserFooter}>
