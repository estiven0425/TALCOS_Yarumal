...
exports.crearControlCalidad = async (req, res) => {
    ...
    try {
        const nuevoControlCalidad = await ControlCalidad.bulkCreate(control_calidad);
...
exports.actualizarControlCalidad = async (req, res) => {
    ...
    try {
        const updatePromises = control_calidad.map(async (control) => {
            const controlCalidad = await ControlCalidad.findByPk(control.id_control_calidad);
            ...
            if (controlCalidad) {
                ...
                throw new Error(`Control de calidad con id ${control.id_control_calidad} no encontrado`);
            }
        });
        ...
        res.status(500).json({ error: 'Error al actualizar los controles de calidad' });
    }
};